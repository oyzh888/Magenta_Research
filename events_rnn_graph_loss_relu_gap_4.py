...
def get_build_graph_fn(mode, config, sequence_example_file_paths=None):
  ...
  def build():
    ...
    if mode == 'train' or mode == 'eval':
      ...
      def extra_loss(prediction_flat):
          max_steps = 19200
          bar_distance = 16 * 3
          first_bar = prediction_flat[:max_steps]
          second_bar = prediction_flat[1:max_steps + 1]
          ...
          def neg_to_non_neg(bar):
              ...
              the positive number will be themselves
              '''
              bar_abs = tf.abs(bar)
          ...
          bar_result = tf.square(bar_with_first - bar_with_last)

          bar_result = tf.split(bar_result, num_or_size_splits=1197, axis=0)
          bar_result = tf.reduce_sum(bar_result, 1)
          threshold = 4
          mask = bar_result > threshold
          bar_result = tf.cast(bar_result, dtype=tf.float32)
          mask = tf.cast(mask, dtype=tf.float32)

          bar_result = tf.multiply(bar_result, mask)
          euclidean = tf.sqrt(tf.reduce_sum(bar_result))

          return euclidean

      if mode == 'train':
        a = 0.01
        extra_loss_result = a * extra_loss(predictions_flat)
